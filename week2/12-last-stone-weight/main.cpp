...
We have a collection of stones, each stone has a positive integer weight. Each
turn, we choose the two heaviest stones and smash them together.  Suppose the
...
public:
    int lastStoneWeight(const vector<int>& stones) {
        priority_queue<int> ordered { stones.begin(), stones.end() };
        while(ordered.size() != 1) {
            const int largest = ordered.top();
            ordered.pop();
            const int secondLargest = ordered.top();
            ordered.pop();
            if(largest != secondLargest) {
                ordered.push(largest - secondLargest);
            }
        }
        return ordered.top();
    }
